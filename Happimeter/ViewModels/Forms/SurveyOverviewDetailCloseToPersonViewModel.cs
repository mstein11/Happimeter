...
namespace Happimeter.ViewModels.Forms
{
	public class SurveyOverviewDetailCloseToPersonViewModel : BaseViewModel
	{
		...
		private int GetProximityLevel(List<ProximityEntry> entries)
		{
			...
			//first try to get proximity by beacon measures
			var cmProximityEntries = entries.Where(x => x.ProximityType == MeasurementItemTypes.ProximityCm);
			if (cmProximityEntries.Any())
			{
				var cmProximity = entries.Where(x => x.ProximityType == MeasurementItemTypes.ProximityCm).Average(x => x.Average);
				if (cmProximity > 6)
				{
					return 3;
				}
